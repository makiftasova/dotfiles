...
## ollama-pull.service && ollama-pull.timer
* *depends*: /usr/bin/ollama
* both `ollama-pull.service` and `ollama-pull.timer` files must be copied into `~/.config/systemd/user` together
* enable with `systemctl enable --user ollama-pull.timer`.
* runs script at `"${HOME}/.local/sbin/ollama-pull"` everyday at 00:00.

## podman-compose@.service
* *depends*: /usr/bin/podman
* autogenerated systemd unit file via `podman compose systemd --action create-unit`
* can be used for running podman pods as systemd services
